...
from sqlalchemy import create_engine, ForeignKey, JSON, TEXT, String, MetaData
...
class StreamerInfo(BaseModel):
    ...
    name: Mapped[str] = mapped_column(String(600), nullable=False)  # streamer 名称
    url: Mapped[str] = mapped_column(String(255), nullable=False)  # 录制的 url
    title: Mapped[str] = mapped_column(String(600), nullable=False)  # 直播标题
    date: Mapped[datetime] = mapped_column(nullable=False)  # 开播时间
    live_cover_path: Mapped[str] = mapped_column(String(260), nullable=False)  # 封面存储路径
...
class FileList(BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)  # 自增主键
    file: Mapped[str] = mapped_column(String(600), nullable=False)  # 文件名
...
class UploadStreamers(BaseModel):
    ...
    title: Mapped[str] = mapped_column(String(300), nullable=True)  # 自定义标题的时间格式, {title}代表当场直播间标题 {streamer}代表在本config里面设置的主播名称 {url}代表设置的该主播的第一条直播间链接
    ...
    dynamic: Mapped[str] = mapped_column(TEXT, nullable=True)  # 粉丝动态
...
class LiveStreamers(BaseModel):
    ...
    remark: Mapped[str] = mapped_column(String(300), nullable=False)  # 对应配置文件中 {streamer} 变量
